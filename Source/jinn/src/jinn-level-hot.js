...
function DoNode(Engine)
{
    ...
    if(Engine.TickNum % 5 !== 0)
        return ;
    Engine.FindNextAddrHashPOW(100);
    for(var L = 0; L < JINN_CONST.MAX_LEVEL_CONNECTION; L++)
    {
        ...
        if(!Child || !Child.Hot)
        {
            ...
            if(LArr)
            {
                ...
                for(var i = 0; i < LArr.length; i++)
                {
                    ...
                    {
                        if(!AddrItem.SendHotConnectPeriod && CompareArr(Engine.IDArr, AddrItem.IDArr) > 0)
                            continue;
        ...
        if(Child.ROOT_NODE)
            continue;
        if(!Engine.InHotStart(Child) && (!Child.Active || Child.Del))
        {
...
function InitClass(Engine)
{
    ...
    Engine.CanSetHot = function (Child)
    {
        ...
        if(ChildWas && ChildWas !== Child && (ChildWas.Hot || Engine.InHotStart(ChildWas)))
        {
            return 0;
        }
        return 1;
