...
function InitClass(Engine)
{
    ...
    Engine.GetBodyByHash2 = function (BlockNum, Hash) {
        console.log('CONSENSUS [(GetBodyByHash2)]', {
            BlockNum: BlockNum,
            Hash: Hash
        });
        if(IsZeroArr(Hash)) {
    ...
    Engine.GetBodyByHash = function (BlockNum,Hash) {
        console.log('CONSENSUS [(GetBodyByHash)]', {
            BlockNum: BlockNum,
            Hash: Hash
        });
        if(IsZeroArr(Hash)) {
    ...
    Engine.PrepareBlockOnReceive = function (Block) {
        console.log('CONSENSUS [(PrepareBlockOnReceive)]', {
            Block: Block
        });
        if(Block.BlockNum >= JINN_CONST.LINK_HASH_PREV_HASHSUM) {
            if(Block.PrevSumHash) {
                console.log("Was Block.PrevSumHash=" + GetHexFromArr(Block.PrevSumHash))
                ToLog("Was Block.PrevSumHash=" + GetHexFromArr(Block.PrevSumHash));
            }
    ...
    Engine.FindHashInStore = function (Store,Name,Hash1,Hash2) {
        console.log('CONSENSUS [(FindHashInStore)]', {
            Store: Store,
            Name: Name,
            Hash1: Hash1,
            Hash2: Hash2
    ...
    Engine.NeedStoreLoad = function (Store,Name,NameNum,BlockNum) {
        console.log('CONSENSUS [(NeedStoreLoad)]', {
            Store: Store,
            Name: Name,
            NameNum: NameNum,
            BlockNum: BlockNum
    ...
    Engine.AddBlockHeader = function (Context,Child,Block,Store) {
        console.log('CONSENSUS [(AddBlockHeader)]', {
            Context: Context,
            Child: Child,
            Block: Block,
            Store: Store
    ...
    Engine.AddBlockBody = function (Context,Child,Block,Store) {
        console.log('CONSENSUS [(AddBlockBody)]', {
            Context: Context,
            Child: Child,
            Block: Block,
            Store: Store
    ...
    Engine.GetFirstEmptyBodyBlockWithCacheCheck = function (BlockHeadNum,BlockSeed) {
        console.log('CONSENSUS [(GetFirstEmptyBodyBlockWithCacheCheck)]', {
            BlockHeadNum: BlockHeadNum,
            BlockSeed: BlockSeed
        });
    ...
    Engine.CanDoNextBodyLoad = function (NodeStatus,BlockHead,BlockSeed,Num) {
        console.log('CONSENSUS [(CanDoNextBodyLoad)]', {
    ...
    Engine.SetStatusInfoH = function (Store,Num) {
        console.log('CONSENSUS [(SetStatusInfoH)]', {
            Store: Store,
            Num: Num
        });
    ...
    Engine.SetStatusInfoB = function (NodeStatus,LoadBlockHead,BlockSeed,Num) {
        console.log('CONSENSUS [(SetStatusInfoB)]', {
            NodeStatus: NodeStatus,
            LoadBlockHead: LoadBlockHead,
            BlockSeed: BlockSeed,
            Num: Num
        });
        if(Num !== 0)
    ...
    Engine.GetLiderArrAtNum = function (BlockNum) {
        console.log('CONSENSUS [(GetLiderArrAtNum)]', {
            BlockNum: BlockNum,
        });
    ...
    Engine.CheckDataMaxLider = function (Data,BlockNum) {
        console.log('CONSENSUS [(CheckDataMaxLider)]', {
            Data: Data,
            BlockNum: BlockNum
        });
    ...
    Engine.CalcHashMaxLider = function (Data,BlockNum) {
        console.log('CONSENSUS [(CalcHashMaxLider)]', {
            Data: Data,
            BlockNum: BlockNum
        });
    ...
    Engine.CalcHashMaxLiderInner = function (Data,BlockNum) {
        console.log('CONSENSUS [(CalcHashMaxLiderInner)]', {
            Data: Data,
            BlockNum: BlockNum
        });
        if(!Data.DataHash) {
    ...
    Engine.CompareMaxLider = function (Data1,Data2) {
        console.log('CONSENSUS [(CompareMaxLider)]', {
            Data1: Data1,
            Data2: Data2
        });
    ...
    Engine.AddHashToMaxLider = function (Data,BlockNum,bFromCreateNew) {
        console.log('CONSENSUS [(AddHashToMaxLider)]', {
            Data: Data,
            BlockNum: BlockNum,
            bFromCreateNew: bFromCreateNew
        });
    ...
    Engine.DoMaxStatus = function (Store) {
        console.log('CONSENSUS [(DoMaxStatus)Store]', {
            Store: Store
        });
    ...
    Engine.CheckAndSaveChainToDB = function (BlockHead,BlockSeed) {
        console.log('CONSENSUS [(CheckAndSaveChainToDB)]', {
            BlockHead: BlockHead,
            BlockSeed: BlockSeed
        });
        ...
        if(BlockSeed.BlockNum > 25 && JINN_WARNING >= 3) {
            ...
            Engine.ToLog("SaveChainToDB: " + BlockInfo(BlockSeed) + " Was Pow=" + (BlockSeedDB ? BlockSeedDB.Power : 0) + "  COUNT=" + Count + StrCheckSum, 3);
        }
    ...
    Engine.CanProcessBlock = function (BlockNum,Step) {
        console.log('CONSENSUS [(CanProcessBlock)]', {
            BlockNum: BlockNum,
            Step: Step
        })
    ...
    Engine.CanProcessMaxHash = function (BlockNum) {
        console.log('CONSENSUS [(CanProcessMaxHash)]', {
            BlockNum: BlockNum
        })
